...
void BluetoothA2DPSource::filter_inquiry_scan_result(
  ...
  if (eir) {
    ...
    if (found) {
      ...
      // This is expected during "scan only" usage (no target selection).
      ESP_LOGD(BT_AV_TAG, "--Result: Target device not found");
    }
  }
}
...
void BluetoothA2DPSource::bt_av_hdl_avrc_ct_evt(uint16_t event, void *p_param) {
  ...
  case ESP_AVRC_CT_CONNECTION_STATE_EVT: {
    ...
    if (rc->conn_stat.connected) {
      ...
      for (auto event : avrc_rn_events) {
        if (esp_avrc_ct_send_register_notification_cmd(tl, event, event_par) !=
            ESP_OK) {
          ESP_LOGE(BT_AV_TAG,
...
void BluetoothA2DPSource::av_hdl_avrc_tg_evt(uint16_t event, void *p_param) {
  ...
  case ESP_AVRC_TG_PASSTHROUGH_CMD_EVT: {
    ...
    if (passthru_command_callback)
      passthru_command_callback(rc->psth_cmd.key_code, rc->psth_cmd.key_state);
    break;
  }
