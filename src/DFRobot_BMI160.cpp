...
int8_t DFRobot_BMI160::SPIInit(struct bmi160Dev *dev) { return BMI160_OK; }
...
int8_t DFRobot_BMI160::setAccelPwr(struct bmi160Dev *dev) {
  ...
  if ((dev->accelCfg.power >= BMI160_ACCEL_SUSPEND_MODE) &&
    ...
    if (dev->accelCfg.power != dev->prevAccelCfg.power) {
      ...
      if (rslt == BMI160_OK) {
        /* Write accel power */
        rslt = DFRobot_BMI160::setRegs(BMI160_COMMAND_REG_ADDR,
                                       &dev->accelCfg.power, 1, dev);
        /* Add delay of 5 ms */
...
int8_t DFRobot_BMI160::processUnderSampling(uint8_t *data,
  ...
  if (rslt == BMI160_OK) {
    if (dev->accelCfg.power == BMI160_ACCEL_LOWPOWER_MODE) {
      ...
      *data = temp | ((1 << 7) & BMI160_ACCEL_UNDERSAMPLING_MASK);
       ...
       * low power mode */
      if (rslt == BMI160_OK) /* Disable the Pre-filter data*/
        rslt = DFRobot_BMI160::setRegs(BMI160_INT_DATA_0_ADDR, &pre_filter, 2,
                                       dev);
    } else {
...
int8_t DFRobot_BMI160::getSensorData(uint8_t type, int16_t *data) {
  ...
  if (type == onlyAccel) {
    ...
    rslt = DFRobot_BMI160::getSensorData((BMI160_ACCEL_SEL | BMI160_GYRO_SEL),
                                         Oaccel, Ogyro, Obmi160);
    if (rslt == BMI160_OK) {
      data[0] = Ogyro->x;
      data[1] = Ogyro->y;
      data[2] = Ogyro->z;
...
int8_t DFRobot_BMI160::getAccelData(int16_t *data, uint32_t *timestamp) {
  int8_t rslt = BMI160_OK;
  rslt = getSensorData((BMI160_ACCEL_SEL | BMI160_TIME_SEL), Oaccel, NULL,
                       Obmi160);
  if (rslt == BMI160_OK) {
    data[0] = Oaccel->x;
    data[1] = Oaccel->y;
...
int8_t DFRobot_BMI160::getGyroData(int16_t *data, uint32_t *timestamp) {
  int rslt = BMI160_OK;
  rslt =
      getSensorData((BMI160_GYRO_SEL | BMI160_TIME_SEL), NULL, Ogyro, Obmi160);
  if (rslt == BMI160_OK) {
    data[0] = Ogyro->x;
    data[1] = Ogyro->y;
    data[2] = Ogyro->z;
...
int8_t DFRobot_BMI160::getAccelGyroData(int16_t *data) {
  int8_t rslt = BMI160_OK;
  rslt = getSensorData((BMI160_ACCEL_SEL | BMI160_GYRO_SEL), Oaccel, Ogyro,
                       Obmi160);
  if (rslt == BMI160_OK) {
    data[0] = Ogyro->x;
    data[1] = Ogyro->y;
...
int8_t DFRobot_BMI160::getAccelGyroData(int16_t *data, uint32_t *timestamp) {
  int8_t rslt = BMI160_OK;
  rslt = getSensorData((BMI160_ACCEL_SEL | BMI160_GYRO_SEL | BMI160_TIME_SEL),
                       Oaccel, Ogyro, Obmi160);
  if (rslt == BMI160_OK) {
    data[0] = Ogyro->x;
    data[1] = Ogyro->y;
...
int8_t DFRobot_BMI160::getSensorData(uint8_t select_sensor,
                                     struct bmi160SensorData *accel,
                                     struct bmi160SensorData *gyro,
                                     struct bmi160Dev *dev) {
  int8_t rslt = BMI160_OK;
  uint8_t time_sel;
  uint8_t sen_sel;
...
int8_t DFRobot_BMI160::getAccelData(uint8_t len, struct bmi160SensorData *accel,
                                    struct bmi160Dev *dev) {
  int8_t rslt;
  uint8_t idx = 0;
  ...
  rslt =
      DFRobot_BMI160::getRegs(BMI160_ACCEL_DATA_ADDR, data_array, 6 + len, dev);
...
int8_t DFRobot_BMI160::getGyroData(uint8_t len, struct bmi160SensorData *gyro,
                                   struct bmi160Dev *dev) {
  int8_t rslt;
  uint8_t idx = 0;
  ...
  if (len == 0) {
    ...
    rslt = DFRobot_BMI160::getRegs(BMI160_GYRO_DATA_ADDR, data_array, 12 + len,
                                   dev);
    if (rslt == BMI160_OK) {
      /* Gyro Data */
      lsb = data_array[idx++];
...
int8_t DFRobot_BMI160::getAccelGyroData(uint8_t len,
                                        struct bmi160SensorData *accel,
                                        struct bmi160SensorData *gyro,
                                        struct bmi160Dev *dev) {
  int8_t rslt;
  ...
  rslt =
      DFRobot_BMI160::getRegs(BMI160_GYRO_DATA_ADDR, data_array, 12 + len, dev);
  if (rslt == BMI160_OK) {
    /* Gyro Data */
    lsb = data_array[idx++];
    msb = data_array[idx++];
...
int8_t DFRobot_BMI160::getRegs(uint8_t reg_addr, uint8_t *data, uint16_t len,
                               struct bmi160Dev *dev) {
  int8_t rslt = BMI160_OK;
  // Null-pointer check
  if (dev == NULL) {
...
int8_t DFRobot_BMI160::I2cGetRegs(struct bmi160Dev *dev, uint8_t reg_addr,
...
int8_t DFRobot_BMI160::setRegs(uint8_t reg_addr, uint8_t *data, uint16_t len,
                               struct bmi160Dev *dev) {
  int8_t rslt = BMI160_OK;
  uint8_t count = 0;
  // Null-pointer check
...
int8_t DFRobot_BMI160::I2cSetRegs(struct bmi160Dev *dev, uint8_t reg_addr,
                                  uint8_t *data, uint16_t len) {
  if ((dev->prevAccelCfg.power == BMI160_ACCEL_NORMAL_MODE) ||
      (dev->prevGyroCfg.power == BMI160_GYRO_NORMAL_MODE)) {
    Wire.beginTransmission(dev->id);
    Wire.write(reg_addr);
    for (int i = 0; i < len; i++) {
      Wire.write(data[i]);
...
int8_t DFRobot_BMI160::SPIGetRegs(struct bmi160Dev *dev, uint8_t reg_addr,
                                  uint8_t *data, uint16_t len) {
  digitalWrite(10, LOW);
  // SPI.transfer(0x6B);
  // SPI.transfer(0x10);
...
int8_t DFRobot_BMI160::SPISetRegs(struct bmi160Dev *dev, uint8_t reg_addr,
                                  uint8_t *data, uint16_t len) {
  return BMI160_OK;
}

int8_t DFRobot_BMI160::setInt(int intNum) { return setInt(Obmi160, intNum); }
...
int8_t DFRobot_BMI160::setIntConfig(struct bmi160IntSettg *intConfig,
                                    struct bmi160Dev *dev) {
  int8_t rslt = BMI160_OK;
...
int8_t DFRobot_BMI160::setAccelStepDetectInt(struct bmi160IntSettg *intConfig,
                                             struct bmi160Dev *dev) {
  int8_t rslt = BMI160_OK;
...
int8_t DFRobot_BMI160::enableStepDetectInt(
    struct bmi160AccStepDetectIntCfg *stepDetectIntCfg, struct bmi160Dev *dev) {
  int8_t rslt;
  uint8_t data = 0;
  uint8_t temp = 0;
  ...
  if (rslt == BMI160_OK) {
    ...
    data = temp | ((stepDetectIntCfg->stepDetectorEn << 3) &
...
int8_t DFRobot_BMI160::setIntrPinConfig(struct bmi160IntSettg *intConfig,
                                        struct bmi160Dev *dev) {
  int8_t rslt;
...
int8_t DFRobot_BMI160::configIntOutCtrl(struct bmi160IntSettg *intConfig,
                                        struct bmi160Dev *dev) {
  int8_t rslt;
  uint8_t temp = 0;
  uint8_t data = 0;
  ...
  if (rslt == BMI160_OK) {
    ...
    if (intConfig->intChannel == BMI160_INT_CHANNEL_1) {
      ...
      data =
          temp | ((intr_pin_sett->outputEn << 3) & BMI160_INT1_OUTPUT_EN_MASK);

      /* Output mode */
      temp = data & ~BMI160_INT1_OUTPUT_MODE_MASK;
      data = temp |
             ((intr_pin_sett->outputMode << 2) & BMI160_INT1_OUTPUT_MODE_MASK);

      /* Output type */
      temp = data & ~BMI160_INT1_OUTPUT_TYPE_MASK;
      data = temp |
             ((intr_pin_sett->outputType << 1) & BMI160_INT1_OUTPUT_TYPE_MASK);
      ...
      data =
          temp | ((intr_pin_sett->outputEn << 7) & BMI160_INT2_OUTPUT_EN_MASK);

      /* Output mode */
      temp = data & ~BMI160_INT2_OUTPUT_MODE_MASK;
      data = temp |
             ((intr_pin_sett->outputMode << 6) & BMI160_INT2_OUTPUT_MODE_MASK);

      /* Output type */
      temp = data & ~BMI160_INT2_OUTPUT_TYPE_MASK;
      data = temp |
             ((intr_pin_sett->outputType << 5) & BMI160_INT2_OUTPUT_TYPE_MASK);

      /* edge control */
      temp = data & ~BMI160_INT2_EDGE_CTRL_MASK;
      data =
          temp | ((intr_pin_sett->edgeCtrl << 4) & BMI160_INT2_EDGE_CTRL_MASK);
    }
...
int8_t DFRobot_BMI160::configIntLatch(struct bmi160IntSettg *intConfig,
                                      struct bmi160Dev *dev) {
  int8_t rslt;
  uint8_t temp = 0;
  uint8_t data = 0;
...
int8_t DFRobot_BMI160::mapFeatureInterrupt(struct bmi160IntSettg *intConfig,
                                           struct bmi160Dev *dev) {
  int8_t rslt;
...
int8_t DFRobot_BMI160::configStepDetect(
    struct bmi160AccStepDetectIntCfg *stepDetectIntCfg, struct bmi160Dev *dev) {
  int8_t rslt;
  uint8_t temp = 0;
  ...
  if (stepDetectIntCfg->stepDetectorMode == BMI160_STEP_DETECT_NORMAL) {
    ...
    data_array[0] = 0x15;
    data_array[1] = 0x03;
  } else if (stepDetectIntCfg->stepDetectorMode ==
    ...
    if (rslt == BMI160_OK) {
      ...
      /* Adding steptimeMin */
      data_array[1] =
          temp | ((stepDetectIntCfg->stepMinBuf) & BMI160_STEP_MIN_BUF_MASK);
    }
  }
...
int8_t DFRobot_BMI160::setStepCounter(uint8_t step_cnt_enable,
                                      struct bmi160Dev *dev) {
  int8_t rslt = BMI160_OK;
  uint8_t data = 0;
