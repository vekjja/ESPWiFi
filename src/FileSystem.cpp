...
void ESPWiFi::srvFS() {
  ...
  HTTPRoute("/api/files", HTTP_OPTIONS, [](httpd_req_t *req) {
  ...
  HTTPRoute("/api/storage", HTTP_OPTIONS, [](httpd_req_t *req) {
    ESPWiFi *espwifi = (ESPWiFi *)req->user_ctx;
    if (espwifi == nullptr) {
      httpd_resp_send_500(req);
      return ESP_FAIL;
  ...
  HTTPRoute("/api/storage", HTTP_GET, [](httpd_req_t *req) {
    ESPWiFi *espwifi = (ESPWiFi *)req->user_ctx;
    if (espwifi == nullptr) {
      httpd_resp_send_500(req);
      return ESP_FAIL;
  ...
  HTTPRoute("/api/files/mkdir", HTTP_OPTIONS, [](httpd_req_t *req) {
    ESPWiFi *espwifi = (ESPWiFi *)req->user_ctx;
    if (espwifi == nullptr) {
      httpd_resp_send_500(req);
      return ESP_FAIL;
  ...
  HTTPRoute("/api/files/mkdir", HTTP_POST, [](httpd_req_t *req) {
    ESPWiFi *espwifi = (ESPWiFi *)req->user_ctx;
    if (espwifi == nullptr) {
      httpd_resp_send_500(req);
      return ESP_FAIL;
    ...
    if (content_len > 512) {
      espwifi->sendJsonResponse(req, 400, "{\"error\":\"Request too large\"}");
      return ESP_OK;
    }
  ...
  HTTPRoute("/api/files/rename", HTTP_OPTIONS, [](httpd_req_t *req) {
    ESPWiFi *espwifi = (ESPWiFi *)req->user_ctx;
    if (espwifi == nullptr) {
      httpd_resp_send_500(req);
      return ESP_FAIL;
  ...
  HTTPRoute("/api/files/rename", HTTP_POST, [](httpd_req_t *req) {
    ESPWiFi *espwifi = (ESPWiFi *)req->user_ctx;
    if (espwifi == nullptr) {
      httpd_resp_send_500(req);
      return ESP_FAIL;
    ...
    if (fsParam.empty() || oldPath.empty() || newName.empty()) {
      espwifi->sendJsonResponse(req, 400, "{\"error\":\"Missing parameters\"}");
      return ESP_OK;
    }
    ...
    if (espwifi->isRestrictedSystemFile(fsParam, oldPath)) {
      espwifi->sendJsonResponse(req, 403,
    ...
    std::string dirPath =
        (lastSlash != std::string::npos) ? oldPath.substr(0, lastSlash) : "/";
  ...
  HTTPRoute("/api/files/delete", HTTP_OPTIONS, [](httpd_req_t *req) {
    ESPWiFi *espwifi = (ESPWiFi *)req->user_ctx;
    if (espwifi == nullptr) {
      httpd_resp_send_500(req);
      return ESP_FAIL;
  ...
  HTTPRoute("/api/files/delete", HTTP_POST, [](httpd_req_t *req) {
    ESPWiFi *espwifi = (ESPWiFi *)req->user_ctx;
    if (espwifi == nullptr) {
      httpd_resp_send_500(req);
      return ESP_FAIL;
    ...
    if (deleteSuccess) {
      ...
      if (dirExists(fullPath)) {
        espwifi->log(INFO, "🗑️  Deleted directory on %s: %s", fsName.c_str(),
                     filePath.c_str());
      } else {
  ...
  HTTPRoute("/api/files/upload", HTTP_OPTIONS, [](httpd_req_t *req) {
    ESPWiFi *espwifi = (ESPWiFi *)req->user_ctx;
    if (espwifi == nullptr) {
      httpd_resp_send_500(req);
      return ESP_FAIL;
  ...
  // handle it manually
  HTTPRoute("/api/files/upload", HTTP_POST, [](httpd_req_t *req) {
    ESPWiFi *espwifi = (ESPWiFi *)req->user_ctx;
    if (espwifi == nullptr) {
      httpd_resp_send_500(req);
      return ESP_FAIL;
    ...
    if (boundary_pos == std::string::npos) {
      espwifi->sendJsonResponse(req, 400, "{\"error\":\"Missing boundary\"}");
      return ESP_OK;
    }
    ...
    if (filename_pos == std::string::npos) {
      espwifi->sendJsonResponse(req, 400, "{\"error\":\"No filename found\"}");
      return ESP_OK;
    }
    ...
    if (filename_end == std::string::npos) {
      espwifi->sendJsonResponse(req, 400, "{\"error\":\"Invalid filename\"}");
      return ESP_OK;
    }
    ...
    size_t written = fwrite(bodyStr.c_str() + data_start, 1, data_len, file);
    fclose(file);

    if (written != data_len) {
      remove(fullFilePath.c_str());
      espwifi->sendJsonResponse(req, 500, "{\"error\":\"File write failed\"}");
      return ESP_OK;
    }
