...
bool appendSpaces = true,
     wpa2 = false;  // makes all SSIDs 32 characters long to improve performance
...
void ESPWiFi::startSSIDSpoof() {
  ...
  randomMac();
  // Update web server routes to use the new API
  webServer->on(
      "/ssidspoof/start", HTTP_OPTIONS,
      [this](AsyncWebServerRequest *request) { handleCorsPreflight(request); });
  webServer->on(
      "/ssidspoof/start", HTTP_POST, [this](AsyncWebServerRequest *request) {
  ...
  webServer->on(
      "/ssidspoof/stop", HTTP_OPTIONS,
      [this](AsyncWebServerRequest *request) { handleCorsPreflight(request); });
  webServer->on(
      "/ssidspoof/stop", HTTP_POST, [this](AsyncWebServerRequest *request) {
        ssidSpoofEnabled = false;
        Serial.println("📶  🛑SSID Spoofing Stopped");
        request->send(200, "application/json", "{\"status\":\"Stopped\"}");
      });
  Serial.println("📶 SSID Spoofing Enabled:");
  Serial.println("\tPOST /ssidspoof/start");
  Serial.println("\tPOST /ssidspoof/stop");
}
